...
def load_and_preprocess_data(file_path):
    """Loads the dataset, removes non-numeric columns, applies scaling, and reduces dimensions using PCA."""
    data = pd.read_csv(file_path)
    X = data.drop(columns=['KMeans_Cluster', 'Agglomerative_Cluster', 'Title', 'Year',
                           'Short Description', 'Directors', 'Star 1', 'Star 2', 'Star 3',
                           'Genre 1', 'Genre 2', 'Genre 3', 'Group', 'Metascore']).fillna(0)
    ...
    return X_pca, Y_kmeans, Y_agglomerative, pca


def train_and_evaluate(X_train, X_test, Y_train, Y_test, model_name):
    """Trains a RandomForest model using RandomizedSearchCV, evaluates performance, and plots feature importance and ROC curve."""
    try:
        smote = SMOTE(random_state=42, k_neighbors=max(1, min(3, min(Y_train.value_counts()) - 1)))
...
file_path = 'nn_cluster.csv'
X_pca, Y_kmeans, Y_agglomerative, pca = load_and_preprocess_data(file_path)
...
train_and_evaluate(X_train, X_test, Y_train_kmeans, Y_test_kmeans, "KMeans")
train_and_evaluate(X_train, X_test, Y_train_agglomerative, Y_test_agglomerative, "Agglomerative")
